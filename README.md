...
## Testing Guidelines
There is no automated test suite in this repo. Validate changes by formatting with `stylua`, starting `nvim`, and running `:checkhealth`. For plugin changes, confirm the affected filetype or workflow manually, for example opening a Go file after editing `lua/plugins/go.lua`.
